function ProjectCard({ projects, singleButton }) {
  return (
    <div
      className={`mt-10 border border-[#AAAAAA] rounded-md w-full px-6 md:px-10 py-6 ${singleButton ? "flex flex-col md:flex-row gap-6" : ""}`}
    >
      <div
      ...
      <div className="pt-6">
        ...
        <p className="pt-3 text-[#5B5A5A] text-sm md:text-base">{projects.description}</p>
        ...
        <div className="flex flex-col sm:flex-row justify-between mt-8 gap-5">
          {!singleButton && (
            <button className="flex-1 relative cursor-pointer overflow-hidden px-4 py-2 md:px-5 md:py-3 rounded-lg bg-[#0F0F0F] text-white group">
          ...
          <button
            className={`flex-1 cursor-pointer px-4 py-2 md:px-5 md:py-3  rounded-lg text-white transition ${
              singleButton ? "bg-black hover:bg-[#EB6200]" : "bg-[#EB6200]"
            }`}
          >
